...
# Simplified version for CDISCPILOT01
...
if (nrow(mh) > 0) {
  admh <- mh %>%
    select(USUBJID, MHSEQ, MHTERM, MHDECOD, MHBODSYS,
...
if (nrow(ex) > 0) {
  adex <- ex %>%
    select(USUBJID, EXSEQ, EXTRT, EXDOSE, EXDOSU,
...
if (nrow(qs) > 0) {
  ...
  qs_eff <- qs %>%
    filter(QSTESTCD %in% c("ADAS11", "CIBIC+", "NPI-X", "MMSE"),
           !is.na(QSSTRESN))
  ...
  if (nrow(qs_eff) > 0) {
    adeff <- qs_eff %>%
      ...
      select(USUBJID, PARAMCD, PARAM, AVISIT, AVAL, VISITNUM, QSDTC) %>%
...
if (nrow(ae) > 0) {
  adae <- ae %>%
    select(USUBJID, AESEQ, AETERM, AEDECOD, AEBODSYS,
           AESER, AEREL, AEOUT, AESEV, AETOXGR,
           AESTDTC, AEENDTC) %>%
    left_join(adsl %>% select(USUBJID, TRT01P, TRT01PN, SAFFL, RFXSTDTC),
              by = "USUBJID") %>%
    mutate(
      TRTEMFL = ifelse(!is.na(AESTDTC) & AESTDTC >= RFXSTDTC, "Y", "N"),
      TRTRELFL = ifelse(AEREL %in% c("POSSIBLE", "PROBABLE"), "Y", "N"),
...
if (nrow(lb) > 0) {
  adlb <- lb %>%
    ...
    select(USUBJID, LBSEQ, LBTESTCD, LBTEST, LBCAT, LBSCAT,
...
if (nrow(vs) > 0) {
  advs <- vs %>%
    ...
    select(USUBJID, VSSEQ, VSTESTCD, VSTEST, VSCAT,
...
cat("Files created:\n")
system(paste("ls -la", output_dir, "| grep -E '\\.(csv|rds)$'"))