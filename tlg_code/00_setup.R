...
required_pkgs <- c(
  ...
  "survival", "ggsurvfit", "emmeans", "broom",
  "haven", "readr", "tools"
)
...
pop_definitions <- list(
  "All Enrolled Subjects" = list(flag = "ENRLFL", value = "Y"),
  "Safety Analysis Set" = list(flag = "SAFFL", value = "Y"),
  "Efficacy Population" = list(flag = "EFFFL", value = "Y"),
  "Intent-to-Treat" = list(flag = "ITTFL", value = "Y")
)

# ---- gtsummary theme ----
# Use default theme for now to avoid errors
...
# Filter population
filter_population <- function(data, population) {
  if (population == "All Enrolled Subjects") {
    return(data %>% filter(ENRLFL == "Y"))
  } else if (population == "Safety Analysis Set") {
    return(data %>% filter(SAFFL == "Y"))
  } else if (population == "Efficacy Population") {
    return(data %>% filter(EFFFL == "Y"))
  } else if (population == "Intent-to-Treat") {
    return(data %>% filter(ITTFL == "Y"))
  } else {
    warning(paste("Unknown population:", population, "- returning unfiltered data"))
    return(data)
  }
}
